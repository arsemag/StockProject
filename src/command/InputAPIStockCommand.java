...
public class InputAPIStockCommand extends ACommand {
  ...
  public void run(IStockMarket myStockMarket) {
    ...
    try {
      ...
      if (inPortfolio) {
        if (myStockMarket.getPortfolio(portfolioName).hasStockAtDate(date, userInstruction)) {
          myStockMarket.getPortfolio(portfolioName).addExistingStock(userInstruction,
                  date, shares);
          return;
        }
      }
